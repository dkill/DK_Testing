...
viewports.forEach((viewport) => {
	describe(`Main navigation: ${viewport.device} (${viewport.width} x ${viewport.height})`, () => {
		...
		context('Menu', () => {
			...
			it.only('Section links are clickable and show the correct content', () => {
			...
			it.only('I can scroll to see all content in each section', () => {
				cy.get('a.tab-handle').each(($el, i, $list) => {
					const sectionID = $el.attr('data-menu-handle')
					cy.wrap($el).click()
		...
		context('Header links', () => {
			...
			it('Clicking on the main nav links forwards me to the correct URLs', () => {
				cy.get('[data-header-main-menu]')
					.find('a')
					.each(($el, i, $list) => {
						let linkText = $el.text().toLowerCase()
						if (linkText !== 'sale') {
							cy.wrap($el).should('have.attr', 'href').and('include', linkText)
						} else {
							cy.wrap($el).should('have.attr', 'href').and('include', 'clearance')
						}
					})
			})
		})
		...
		context('Cart', () => {
			...
			it('Cart box background color is different when cart is empty vs not empty', { scrollBehavior: false }, () => {
				...
				cy.get('[data-quickview-container]').first().find('button').click()
